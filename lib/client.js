			...
			return el("div", { className: "dof-root" },
				...
				el("div", { className: "dof-card" },
					el("div", { className: "dof-row" },
						el("span", { className: "k" }, t.toastLabel),
						el("button", {
							type: "button",
							role: "switch",
					...
					el("p", { className: "dof-note" }, t.toastHint)
				),
			...
			try {
				...
				if (document.getElementById(id) === null) {
					...
					tag.textContent = [
						...
						".dof-switch{position:relative;flex:none;width:36px;height:20px;border-radius:10px;border:1px solid var(--dsw-alias-border-l1,#3d3d45);background:var(--dsw-alias-bg-layer-2,#2b2b31);cursor:pointer;padding:0;transition:background .15s}",
						".dof-switch.on{background:var(--dsw-alias-brand-primary,#3d5afe)}",
						".dof-switch-knob{position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:var(--dsw-alias-label-primary,#f2f2f2);transition:left .15s}",
						".dof-switch.on .dof-switch-knob{left:18px}",
