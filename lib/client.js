...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const { createElement: el, useState } = React;
		...
		const T = {
			zh: {
				...
				errNone: "（暂无错误）",
				testTitle: "测试",
				testHint: "直接解析当前会话的工作目录并调用宿主打开，绕过侧边栏菜单，用于定位问题。",
				testBtn: "测试打开当前会话文件夹",
				testRunning: "正在打开…",
				testOk: "测试成功，已请求宿主打开：",
				testFail: "测试失败：",
				noteTitle: "说明",
				noteText: "会话行 ⋯ 菜单由内置 UI 渲染，无官方菜单项扩展槽位，本插件采用注入方式实现；本页记录每次操作的详细日志，若菜单项未出现或点击无反应，可在此查看原因（宿主能力、解析路径、RPC 错误等）。",
				item: "打开文件夹",
			...
			en: {
				...
				sectionTitle: "Open folder · Diagnostics",
				statusTitle: "Status",
				verLabel: "Plugin version",
				capLabel: "Host can open local paths",
				capYes: "Yes",
				capNo: "No (menu item disabled)",
				capUnknown: "detecting…",
				statsLabel: "Opened successfully",
				failStatsLabel: "Failed",
				lastOpenedLabel: "Last opened",
				lastOpenedNone: "(none)",
				menuItemState: "Menu item",
				menuItemOn: "injected",
				menuItemOff: "disabled",
				evtTitle: "Recent events",
				errTitle: "Errors",
				errNone: "(no errors)",
				testTitle: "Test",
				testHint: "Resolve the current session's folder and ask the host to open it, bypassing the sidebar menu — to isolate problems.",
				testBtn: "Test open current session folder",
				testRunning: "opening…",
				testOk: "Test succeeded, host was asked to open:",
				testFail: "Test failed:",
				noteTitle: "Notes",
				noteText: "The session-row ⋯ menu is rendered by the built-in UI with no public menu-item slot, so this plugin injects the item. This page logs every operation; if the menu item is missing or a click does nothing, the reason (host capability, resolved path, RPC error) is shown here.",
				item: "Open folder",
		...
		/** Settings-page diagnostics component. Props: { useDiagnostics, api }. */
		function OpenFolderSection(props) {
			...
			const fmt = (at) => {
				try { return new Date(at).toLocaleTimeString(); } catch { return String(at); }
			};
			const status = diag?.status ?? { ready: false, canOpenPath: false, describeError: null };
			const stats = diag?.stats ?? { opened: 0, failed: 0 };
			const events = diag?.events ?? [];
			const errors = diag?.errors ?? [];
			const lastOpened = diag?.lastOpened ?? null;
			const capText = !status.ready ? t.capUnknown : (status.canOpenPath ? t.capYes : t.capNo);
			const capClass = !status.ready ? "dof-neu" : (status.canOpenPath ? "dof-ok" : "dof-bad");
			...
			return el("div", { className: "dof-root" },
				el("p", { className: "dof-note" }, t.noteText),

				el("div", { className: "dof-card" },
					el("h4", { className: "dof-h" }, t.statusTitle),
					el("div", { className: "dof-row" }, el("span", { className: "k" }, t.verLabel), el("span", null, "v" + VERSION)),
					el("div", { className: "dof-row" }, el("span", { className: "k" }, t.capLabel), el("span", { className: capClass }, capText)),
					el("div", { className: "dof-row" }, el("span", { className: "k" }, t.menuItemState), el("span", { className: status.canOpenPath ? "dof-ok" : "dof-neu" }, status.canOpenPath ? t.menuItemOn : t.menuItemOff)),
					el("div", { className: "dof-row" }, el("span", { className: "k" }, t.statsLabel), el("span", null, String(stats.opened))),
					el("div", { className: "dof-row" }, el("span", { className: "k" }, t.failStatsLabel), el("span", { className: stats.failed > 0 ? "dof-bad" : "" }, String(stats.failed))),
					el("div", { className: "dof-row" }, el("span", { className: "k" }, t.lastOpenedLabel),
						lastOpened !== null
				...
				el("div", { className: "dof-card" },
					el("h4", { className: "dof-h" }, t.testTitle),
					el("p", { className: "dof-note" }, t.testHint),
					el("div", { className: "dof-testrow" },
						el("button", { type: "button", className: "dof-btn", disabled: busy || !status.canOpenPath, onClick: onTest }, busy ? t.testRunning : t.testBtn)),
					diag?.test !== null && diag?.test !== void 0
						? el("div", { className: "dof-testresult " + (diag.test.ok ? "dof-ok" : "dof-bad") },
							diag.test.ok ? t.testOk + " " + diag.test.path : t.testFail + (diag.test.error ?? "unknown"))
						: null
				),

				el("div", { className: "dof-card" },
					el("h4", { className: "dof-h" }, t.errTitle),
					errors.length === 0
						? el("div", { className: "dof-evt" }, t.errNone)
						: el("div", null, errors.map((e, i) => el("div", { key: "e" + String(i), className: "dof-evt err" },
							el("span", { className: "tm" }, fmt(e.at)), e.text)))
				),

				el("div", { className: "dof-card" },
					el("h4", { className: "dof-h" }, t.evtTitle),
					el("div", null, events.map((e, i) => el("div", { key: "v" + String(i), className: "dof-evt " + e.kind },
						el("span", { className: "tm" }, fmt(e.at)), e.text)))
				)
			);
		}
